...
**常见参数**：
```bash
--model gemini         # Gemini生图模型（默认）
--model seedream       # SeedDream生图模型
--proxy http://...     # 设置代理（如需要）
```

**错误处理**：
- API密钥未配置 → 检查环境变量 `OPENROUTER_API_KEY`
- 生成失败 → 简化提示词，减少复杂要求；或换模型重试
- 中文乱码 → 强化提示词中的中文要求，或换模型
