...
- **[api-configuration.md](references/api-configuration.md)** - 生图API配置指南（OpenRouter + OPENROUTER_API_KEY）
...
### 实用脚本
- **scripts/generate_image.py** - 生图API调用脚本（通过OpenRouter，支持gemini/seedream模型）
...
**⚠️ 生成图片前，必须先通过 AskUser 询问用户选择模型（gemini 或 seedream）**
...
# 步骤5：生成封面图（强制）
# ⚠️ 生图前必须先通过AskUser询问用户选择模型（gemini或seedream）
generate_image(
  prompt="Claude Sonnet 4 封面，蓝紫渐变，标题+副标题...",
  model="gemini",  # 或 seedream，由用户选择
  output="cover.png"
)
