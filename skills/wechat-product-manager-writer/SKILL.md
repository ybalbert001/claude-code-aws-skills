...
4. **封面图是强制要求**
   ...
   - **生图前必须通过 AskUser 询问用户选择模型（gemini 或 seedream）**（详见步骤 6）
...
通过 AskUser 询问用户选择生图模型，两个模型均通过 OpenRouter 调用：

| 模型别名 | 实际模型 | 说明 |
|---------|---------|------|
| `gemini` | google/gemini-3.1-flash-image-preview | 默认，中文效果较好 |
| `seedream` | bytedance-seed/seedream-4.5 | 备选，画面质感好 |

**环境变量**：确保设置了 `OPENROUTER_API_KEY`
